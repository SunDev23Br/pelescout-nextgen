...
function ClubesPage() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      const userIds = Array.from(new Set(cands.map((c) => c.user_id).filter((v): v is string => !!v)));
      const peneiraIds = Array.from(new Set(cands.map((c) => c.peneira_id).filter((v): v is string => !!v)));
      ...
      const mapped: AtletaAprovado[] = cands.map((c) => {
