...
function PerfilOlheiroPage() {
  ...
  useEffect(() => {
    ...
    (async () => {
      const [{ data: prof, error }, avals, aprovados, peneiras] =
        await Promise.all([
          supabase
            .from("profiles")
          ...
          supabase
            .from("avaliacoes")
            .select("candidato_id, nota_geral, decisao")
      ...
      if (prof) setProfile(prof as ScoutProfile);

      const rows = avals.data ?? [];
      const notas = rows
        .map((r) => Number(r.nota_geral))
        .filter((n) => Number.isFinite(n) && n > 0);
      setStats({
        observados: new Set(rows.map((r) => r.candidato_id).filter(Boolean))
          .size,
        avaliacoes: rows.length,
        indicacoes: rows.filter((r) => r.decisao === "aprovar").length,
  ...
  const nota = useMemo(() => {
    ...
    // notas do sistema são 0–10, exibimos em escala 0–5
            ...
            <Card title="Sobre o olheiro">
              {profile.bio ? (
                ...
                <p className="text-sm italic text-muted-foreground">
                  ...
                  jovens jogadores. Adicione sua biografia em{" "}
